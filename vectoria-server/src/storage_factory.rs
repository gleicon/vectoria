        ...
        "edgestore-hnsw" | "edgestore" => {
            ...
            match (repl_bind, repl_primary) {
                (Some(bind_addr), _) => {
                    ...
                    let engine = Arc::clone(re.engine());
                    let store = Arc::new(EdgeStoreStorage::from_mutex_engine(Arc::clone(&engine)));
                    let vidx = Arc::new(
                        EdgeStoreVectorIndex::from_mutex_engine(
                            engine,
                ...
                (None, None) => {
                    // Standalone: RwLock allows concurrent reads during reindex.
